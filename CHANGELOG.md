...
### Fixed
- **Stop-hook guardrail no longer blocks sessions over stray untracked files** — The end-of-session guardrail previously treated every line of `git status --porcelain` as unshipped work, including untracked files (e.g. local scratch, env files, IDE artifacts). Sessions in worktrees with such files looped on stop and produced unhelpful "work is complete" replies. The guardrail now only considers tracked modifications and unpushed commits; anything the agent itself intends to ship will be staged and still surface. ([CYPACK-1194](https://linear.app/ceedar/issue/CYPACK-1194), [#1202](https://github.com/cyrusagents/cyrus/pull/1202))
