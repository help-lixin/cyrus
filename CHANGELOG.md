...
### Changed
- opencode-runner: Simplified `evaluatePermission` to always allow all tools (except built-in denied tools like `question`, `doom_loop`, `external_directory`). This aligns opencode-runner with other runners (claude-runner, cursor-runner, codex-runner, gemini-runner) which delegate tool permission handling to their respective SDKs/CLIs. Git operations now work correctly in opencode-runner sessions.
