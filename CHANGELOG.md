...
### Added
- On Cyrus cloud, each Bash command the agent runs is now executed under a per-command memory budget so a single runaway command can be stopped without taking down the whole session; if a command is killed for exceeding its budget, the event is reported back to the control plane. This is cloud-only and has no effect on self-host or community installs. ([CYPACK-1274](https://linear.app/ceedar/issue/CYPACK-1274), [#PR](https://github.com/cyrusagents/cyrus/pull/PR))
