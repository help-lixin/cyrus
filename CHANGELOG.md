...
### Changed
- **Updated `@anthropic-ai/claude-agent-sdk` to v0.3.146 and `@anthropic-ai/sdk` to `^0.97.1`** — Bumps the bundled Claude Code binary to v2.1.146 and the Anthropic TypeScript SDK to `^0.97.1`. Also refreshes the tool allowance list in `packages/claude-runner/src/config.ts`: adds `LSP`, removes `RemoteTrigger` (no longer shipped in Claude Code SDK v0.3.146). Adds `diagnostics: null` to `BetaMessage` literals in codex-runner, cursor-runner, and gemini-runner (new required field in `@anthropic-ai/sdk` 0.97.x). See [SDK changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md) for full details. ([CYPACK-1228](https://linear.app/ceedar/issue/CYPACK-1228))
