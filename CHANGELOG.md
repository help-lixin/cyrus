...
### Added
- **Per-thread auto-memory for Slack chat sessions** — Slack-triggered chat sessions now get their own persistent Claude auto-memory directory, namespaced per Slack thread (`<cyrusHome>/slack-workspaces/<thread-key>/memory/`). Memory persists across resumes in the same thread without leaking between threads. ([CYPACK-1190](https://linear.app/ceedar/issue/CYPACK-1190))
