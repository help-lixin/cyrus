...
- `AppServerCodexBackend`: drives Codex via the `codex app-server` JSON-RPC protocol (newline-delimited JSON-RPC 2.0 over stdio) with `initialize`→`thread/start`/`thread/resume`→`turn/start` lifecycle, app-server v2 item translation (camelCase→normalized), and `turn/steer` for mid-turn input injection. Includes replay tests built from real `codex app-server` transcripts and an injectable client (`IAppServerClient`) for hermetic backend tests. Experiment scripts and a full requirements/design scope live in `packages/codex-runner/experiments/`.
