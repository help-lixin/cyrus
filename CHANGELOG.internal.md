...
- Added `destroy()` to `AgentSessionResult` in `cyrus-agent-runtime` — equates to ComputeSDK's `ProviderSandbox.destroy()` for ComputeSDK-backed providers (deletes the remote sandbox, releases compute resources) and is a no-op for the local provider. Idempotent and shared with `AgentSession.stop()`, so callers can release the sandbox via either method (or both) without double-destroying. Lets consumers hold only the result, consume the events/result, and tear down without keeping a session reference.
